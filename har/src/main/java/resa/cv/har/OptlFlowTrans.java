 ...
 */
public class OptlFlowTrans extends BaseRichBolt implements Constants {
    ...
    public void execute(Tuple tuple) {
        ...
        for (int i = 0; i < targetComponentTasks.size(); i++) {
            ...
            collector.emitDirect(tID, STREAM_OPT_FLOW, tuple, new Values(frameId, group.get(i),
                    new Protocal.TwoIntegers(flow.width(), flow.height())));
        }
    ...
    public void declareOutputFields(OutputFieldsDeclarer outputFieldsDeclarer) {
        outputFieldsDeclarer.declareStream(STREAM_OPT_FLOW, true, new Fields(FIELD_FRAME_ID, FIELD_FRAME_MAT,
                FIELD_WIDTH_HEIGHT));
    }
}
